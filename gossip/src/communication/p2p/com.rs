...
// pub type ComReceiver = mpsc::Receiver<(Vec<u8>, SocketAddr)>;
// pub type ComSender = mpsc::Sender<(Vec<u8>, SocketAddr)>;
...
//     ) -> Result<(Self, ComReceiver), Box<dyn Error>> {
...
//         let socket = Arc::new(Tcp::bind(addr).await?);
