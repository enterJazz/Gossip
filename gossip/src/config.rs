use std::{path::PathBuf, io, net::SocketAddr, collections::HashMap};
use ini::Ini;
use thiserror::Error;
use url::Url;
...
impl Config {
    ...
    pub fn load_config(path_to_config: PathBuf) -> Result<Self, ConfigError> {
        ...
        let conf = Ini::load_from_file(path_to_config.clone()).map_err(|e| ConfigError::LoadingError { path: path_to_config, err_msg: e.to_string() })?;
        let host_key_path = conf
            .general_section()
            .get(HOST_KEY_PATH_FIELD)
            .ok_or(ConfigError::FieldMissing { field_name: HOST_KEY_PATH_FIELD.to_string() })?
            .parse::<PathBuf>()
            .map_err(|e| ConfigError::ParsingError { field_name: HOST_KEY_PATH_FIELD.to_string(), err_msg: e.to_string() })?;

        let gossip_section = conf.section(Some(GOSSIP_SECTION_NAME))
            .ok_or(ConfigError::SectionMissing { section_name: GOSSIP_SECTION_NAME.to_string() })?;
        ...
        for field_key in [parsed_usize_keys.clone(), parsed_url_keys.clone(), parsed_address_keys.clone()].concat() {

            let field_value = gossip_section.get(&field_key)
                .ok_or(ConfigError::FieldMissing { field_name: field_key.clone() })?;

            if parsed_usize_keys.contains(&field_key) {
                let usize_parsed_value: usize = field_value
                    .parse::<usize>()
                    .map_err(|e| ConfigError::ParsingError { field_name: field_key.clone(), err_msg: e.to_string() })?;
                let _ = parsed_usize_fields.insert(field_key.clone(), usize_parsed_value);
            } else if parsed_url_keys.contains(&field_key) {
                let url_parsed_value: Url = field_value
                    .parse::<Url>()
                    .map_err(|e| ConfigError::ParsingError { field_name: field_key.clone(), err_msg: e.to_string() })?;
...
#[cfg(test)]
mod tests {
    use super::{Config, HOST_KEY_PATH_FIELD, CACHE_SIZE_CONFIG_FIELD, DEGREE_CONFIG_FIELD, BOOTSTRAPPER_CONFIG_FIELD, P2P_ADDRESS_CONFIG_FIELD, API_ADDRESS_CONFIG_FIELD, GOSSIP_SECTION_NAME};
    ...
    fn test_load_config() {
        ...
        assert_eq!(&ini_conf.general_section().get(HOST_KEY_PATH_FIELD).unwrap().parse::<PathBuf>().unwrap(), generated_config.get_host_key_path());

        let gossip_conf = ini_conf.section(Some(GOSSIP_SECTION_NAME)).unwrap();
        assert_eq!(gossip_conf.get(CACHE_SIZE_CONFIG_FIELD).unwrap().parse::<usize>().unwrap(), generated_config.get_cache_size());
        assert_eq!(gossip_conf.get(DEGREE_CONFIG_FIELD).unwrap().parse::<usize>().unwrap(), generated_config.get_degree());
        assert_eq!(&gossip_conf.get(BOOTSTRAPPER_CONFIG_FIELD).unwrap().parse::<Url>().unwrap(), generated_config.get_bootstrapper());
        assert_eq!(gossip_conf.get(P2P_ADDRESS_CONFIG_FIELD).unwrap().parse::<SocketAddr>().unwrap(), generated_config.get_p2p_address());
        assert_eq!(gossip_conf.get(API_ADDRESS_CONFIG_FIELD).unwrap().parse::<SocketAddr>().unwrap(), generated_config.get_api_address());
