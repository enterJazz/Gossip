...
const TEST_CONFIG_PATH_BOOTSTRAPPER_TEMPLATE: &str =
    "tests/resources/configs/integration_config_bootstrapper_template.ini";
const TEST_CONFIG_PATH_PEER_1_TEMPLATE: &str =
    "tests/resources/configs/integration_config_peer_1_template.ini";
const TEST_CONFIG_PATH_PEER_2_TEMPLATE: &str =
    "tests/resources/configs/integration_config_peer_2_template.ini";
const TEST_CONFIG_PATH_PEER_3_TEMPLATE: &str =
    "tests/resources/configs/integration_config_peer_3_template.ini";
    ...
    pub mod gossip {
        ...
        fn setup(action: Action) -> GossipClientConfig {
            ...
            let target_addr = match action {
                // case: API action
                Action::Announce | Action::Notify => gossip_conf
                    .get(API_FIELD)
                    .unwrap()
                    .parse::<SocketAddr>()
                    .unwrap(),
        ...
        async fn run_gossip_api_client(conf: GossipClientConfig) {
            ...
            client_cmd.arg(GOSSIP_CLIENT_PATH);
    ...
    pub mod rps {
        ...
        pub async fn start_rps() {
            ...
            utils::wait_for_service(
                format!("{}:{}", &rps_config.server_address, &rps_config.server_port),
                Duration::from_secs(SERVICE_TIMEOUT_DURATION_SECS),
            );
        }
...
pub mod utils {
    ...
    use super::{
        GENERAL_HOST_KEY, TEST_CONFIG_PATH_BOOTSTRAPPER_TEMPLATE, TEST_CONFIG_PATH_PEER_1_TEMPLATE,
        TEST_CONFIG_PATH_PEER_2_TEMPLATE, TEST_CONFIG_PATH_PEER_3_TEMPLATE,
    };

    use super::{
        TEST_CONFIG_PATH_BOOTSTRAPPER, TEST_CONFIG_PATH_PEER_1, TEST_CONFIG_PATH_PEER_2,
        TEST_CONFIG_PATH_PEER_3,
    };
    ...
    pub fn wait_for_service(address: String, timeout: Duration) {
        ...
        loop {
            match TcpStream::connect(address.clone()) {
                ...
                Err(e) => warn!(
                    "couldn't connect to service at {}: {}\nwaiting...",
                    address.clone(),
                    e.to_string()
                ),
            }
    ...
    fn get_peer_x_config(x: usize) -> config::Config {
        let (peer_conf_path, peer_conf_template) = match x {
            0 => (
                TEST_CONFIG_PATH_BOOTSTRAPPER,
                TEST_CONFIG_PATH_BOOTSTRAPPER_TEMPLATE,
            ),
    ...
    fn init_config_ini(config_template_path: &str, config_target_path: &str) {
        ...
        let adjusted_path = canonicalize_relative_path_to_repo_dir(
            conf.general_section().get(GENERAL_HOST_KEY).unwrap(),
        );
        conf.with_general_section()
            .set(GENERAL_HOST_KEY, adjusted_path.to_str().unwrap());
...
pub mod common_gossip {
    ...
    pub async fn start_gossip_bootstrapper() {
        let config_path =
            utils::canonicalize_relative_path_to_repo_dir(TEST_CONFIG_PATH_BOOTSTRAPPER);
        ...
        utils::wait_for_service(
            config.get_api_address().to_string(),
            Duration::from_secs(SERVICE_TIMEOUT_DURATION_SECS),
        );
        utils::wait_for_service(
            config.get_p2p_address().to_string(),
            Duration::from_secs(SERVICE_TIMEOUT_DURATION_SECS),
        );
    }
}
